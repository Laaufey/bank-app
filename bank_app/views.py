...
def send_request(request, url):
    ...
    response = session.post(url + 'external-transfer/',
                            data=request.POST, headers=headers)
    response.raise_for_status()
...
def transfer(request):
    if request.method == "POST":
        ...
        if transfer_form.is_valid():
            ...
            credit_transfer_path = Bank.objects.get(
                pk=credit_bank_id).transfer_path
            ...
            if credit_bank_id == 1:
                ...
                if r.status_code == 200:
                    transfer = ExternalLedger.transfer(
                        amount, debit_account, debit_text, credit_bank_id)
                    print(transfer)
    else:
    ...
    if request.method == "POST":
        ...
        if account_form.is_valid():
            Account.objects.create(
                user=account_form.cleaned_data['user'], title=account_form.cleaned_data['title'])
    context = {
...
class TransferView(View):
    ...
    def post(self, request, *args, **kwargs):
        ...
        if transfer_form.is_valid():
            ...
            amount = transfer_form.cleaned_data['amount']
            transfer = ExternalLedger.transfer(
                amount, credit_account, credit_text, False)
            print(transfer)
            return HttpResponse(self.test)
